...
class RteProcessor(DataProcessor):
    ...
    def get_MRPC_file(self, filename):
        '''
        ...
        for row in readfile:
            if line_co>0:
                ...
                text_a = line[-2].strip()
                text_b = line[-1].strip()
                label = line[0].strip() #["entailment", "not_entailment"]
                examples.append(
...
def main():
    ...
    train_examples = processor.get_MRPC_file('/export/share/wenpeng/glue_data/MRPC/train.tsv') #train_pu_half_v1.txt
                    ...
                    for idd, dev_or_test_dataloader in enumerate([dev_dataloader, test_dataloader]):
                        ...
                        for k in range(len(pred_label_ids)):
                            if pred_label_ids[k] == gold_label_ids[k]:
                                hit_co +=1
                                if pred_label_ids[k] == 1:
                                    overlap+=1
                        test_acc = hit_co/len(gold_label_ids)

                        epsilon =1e-8
                        recall = overlap/(epsilon+sum(gold_label_ids))
                        precision = overlap/(epsilon+sum(pred_label_ids))
                        test_f1 = 2*recall*precision/(epsilon+recall+precision)

                        test_result = (test_acc, test_f1)

                        if idd == 0: # this is dev
                            if test_f1 > max_dev_f1:
                                max_dev_f1 = test_f1
                                print('\ndev result:', test_result, ' max_dev_f1:', max_dev_f1, '\n')
                            ...
                            final_test_performance = test_result
                            print('\ntest result:', test_result, ' max_test_f1:', max_test_f1, '\n')
